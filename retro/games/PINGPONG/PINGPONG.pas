...
{ РЕМОНТ: PINGPONG ШВИДШИЙ ЗА РЕШТУ — власний пін, не глобальна ручка.
  13.08.2026 Ярик попросив одним реченням дві протилежні речі: «тепер давай
  зробимо футбол ще на 50% повільніше, а пінпонг — на 50% швидше». Обидві гри
  названо прямо, тож обидві половини — піни в тілах самих ігор; глобальні
  crt.MinDelayMs=320 / crt.DelayScale=0.064 лишаються шести іншим бандлам.
  ...
  0.064 це round(1.92)=2 мс, тобто воно ЛЕЖИТЬ НА ПІДЛОЗІ й темп гри — це рівно
  MinDelayMs. Отже «на 50% швидше» — це 320 / 1.5 = 213 мс (4.7 к/с проти
  колишніх 3.1). DelayScale ділимо на ті самі 1.5: він тут НЕ інертний, бо один
  виклик таки перевищує підлогу — delay(duration*500)=7500 на екрані Game Over
  давав 480 мс, а тепер дає 320. Тобто пришвидшується вся гра, а не лише кадр.

  ⚠️ ЦЕ РУХ У БЕЗПЕЧНИЙ БІК, І ЦЕ ВАРТО СКАЗАТИ ВГОЛОС. Застереження в crt.pas
  («320 мс = 3.1 к/с, для кадрових ігор далі не можна») — про СПОВІЛЬНЕННЯ:
