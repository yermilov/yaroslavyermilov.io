...
begin
{ РЕМОНТ: PINGPONG ШВИДШИЙ ЗА РЕШТУ — власний пін, не глобальна ручка.
  13.08.2026, крок перший: Ярик попросив одним реченням дві протилежні речі —
  «тепер давай зробимо футбол ще на 50% повільніше, а пінпонг — на 50% швидше».
  Обидві гри названо прямо, тож обидві половини — піни в тілах самих ігор, а
  глобальні (тоді 320/0.064) лишились решті бандлів.
  Механіка та сама, що у WARWORK і FOOTBALL: у crt це типізовані константи
  (var), а Delay перечитує їх ПРИ КОЖНОМУ виклику, тож присвоєння тут перекриває
  глобальні числа рівно для цього бандла.

  13.08.2026, зігравши ту збірку: «чудово! тепер cars, pingpong, quiditch на
  50% швидше» — тобто пінпонг просять пришвидшити ВДРУГЕ за день, тож 213 -> 142
  і 0.042667 -> 0.028444.

  ⚠️ ЦЕЙ ПІН ОБОВ'ЯЗКОВО РУХАТИ РАЗОМ З ГЛОБАЛЬНИМИ, і саме тому він тут окремо
  згаданий. Того ж дня глобальні поїхали 320 -> 213 (бо cars і quidditch просили
  в тому ж реченні). Якби пін лишився 213, він би зрівнявся з новими глобальними
  і пінпонг НЕ пришвидшився б узагалі — прохання виконалось би для двох ігор із
  трьох, причому мовчки. Пін завжди має бути глобальні/1.5, поки пінпонг тримає
  на один крок більше за решту.

  Чому саме ці числа: кожне очікування — max(round(ms*DelayScale), MinDelayMs).
  Темп самої гри задає ОДИН виклик — await(delay(duration*2)) у BallWalking, а
  duration=15 (чотирнадцяте число в data/OPTIONS.COD), тобто delay(30). При
  0.028444 це round(0.85)=1 мс, тобто воно ЛЕЖИТЬ НА ПІДЛОЗІ й темп гри — це
  рівно MinDelayMs. Отже «на 50% швидше» — це 213 / 1.5 = 142 мс (7.0 к/с проти
  4.7). DelayScale ділимо на ті самі 1.5: він тут НЕ інертний, бо один виклик
  таки перевищує підлогу — delay(duration*500)=7500 на екрані Game Over давав
  320 мс, а тепер дає 213. Тобто пришвидшується вся гра, а не лише кадр.

  ⚠️ ЦЕ РУХ У БЕЗПЕЧНИЙ БІК, І ЦЕ ВАРТО СКАЗАТИ ВГОЛОС. Застереження в crt.pas
  («3.1 к/с, для кадрових ігор далі не можна») — про СПОВІЛЬНЕННЯ:
  у PINGPONG одне число є і темпом, і частотою малювання, тож сповільнення
  неминуче робить із гри слайдшоу. Прискорення тією самою ручкою навпаки
  повертає кадри: м'яч робить менший стрибок між перемальовуваннями, тобто гра
  стає і швидшою, і плавнішою. Стеля з'явиться лише знизу, коли крок м'яча за
  кадр (BallSpeed=30 у OPTIONS.COD) стане надто дрібним, щоб рухатись — а до
  цього ще далеко.
