...
// pulls the current conditions from Open-Meteo (free, no API key), and shows the
// temperature + condition. The WMO-code → label/glyph map is ported from plun's
// WeatherPromptService vocabulary. A later increment paints an AI scene from the
// same conditions; until then this is the keyless data layer.
...
async function fetchWeather(lat: number, lon: number): Promise<Omit<Current, 'place' | 'fallback'>> {
  const url =
...
export default function WeatherWidget() {
  ...
  const run = (lat: number, lon: number, place: string, fallback: boolean) => {
    ...
    fetchWeather(lat, lon)
      .then((w) => setState({ status: 'ready', data: { ...w, place, fallback } }))
      .catch((e: unknown) =>
