...
rtl.module("program",["System","crt","dos","nls","hpteams"],function () {
  ...
  this.Main = async function () {
    ...
    $mod.s2 = 0;
    pas.crt.Randomize();
    pas.hpteams.HpHeader(pas.nls.Loc("match simulator  ·  fully automatic","симулятор матчу  ·  повністю автоматичний"));
    ...
    if ($mod.home === 0) {
      ...
      $mod.h1 = pas.System.Trunc(await pas.crt.AskReal(pas.nls.Loc("Team 1 accuracy (number, e.g. 500)","Влучність команди 1 (число, напр. 500)")));
    } else {
      $mod.t1 = pas.hpteams.HpName($mod.home);
      $mod.p1 = pas.hpteams.HpSeeker($mod.home);
      $mod.r1 = pas.hpteams.HpReaction($mod.home);
      $mod.h1 = pas.hpteams.HpAccuracy($mod.home);
    };
    ...
    if (pas.hpteams.HpPick === 0) {
      ...
      $mod.h2 = pas.System.Trunc(await pas.crt.AskReal(pas.nls.Loc("Team 2 accuracy (number, e.g. 500)","Влучність команди 2 (число, напр. 500)")));
    } else {
      ...
      $mod.h2 = pas.hpteams.HpAccuracy(pas.hpteams.HpPick);
    };
    pas.crt.ClrScr();
    ...
    do {
      ...
      if (!$mod.shown || (((($mod.t - $mod.t0) % (pas.System.Random(5) + 1)) === 0) && (($mod.t - $mod.t0) !== 0))) {
        ...
        if ((($mod.t - $mod.t0) % 60) < 10) {
         ...
         else pas.System.Writeln(Math.floor(($mod.t - $mod.t0) / 60),":",($mod.t - $mod.t0) % 60);
        pas.System.Writeln($mod.t1);
        pas.System.Write($mod.t2);
        pas.crt.GotoXY(40,2);
      ...
      await pas.crt.Delay(pas.System.Random(6) * 5000);
      $mod.g1 = ($mod.n - pas.System.Random(1000 - $mod.h1)) + pas.System.Random(1000 - $mod.h1);
      $mod.g2 = ($mod.n - pas.System.Random(1000 - $mod.h2)) + pas.System.Random(1000 - $mod.h2);
      if (Math.abs($mod.n - $mod.g1) <= $mod.h1) {
        pas.crt.ClrScr();
        if ((($mod.t - $mod.t0) % 60) < 10) {
          pas.System.Writeln(Math.floor(($mod.t - $mod.t0) / 60),":0",($mod.t - $mod.t0) % 60)}
         else pas.System.Writeln(Math.floor(($mod.t - $mod.t0) / 60),":",($mod.t - $mod.t0) % 60);
      ...
      if (Math.abs($mod.n - $mod.g2) <= $mod.h2) {
        pas.crt.ClrScr();
        if ((($mod.t - $mod.t0) % 60) < 10) {
          pas.System.Writeln(Math.floor(($mod.t - $mod.t0) / 60),":0",($mod.t - $mod.t0) % 60)}
         else pas.System.Writeln(Math.floor(($mod.t - $mod.t0) / 60),":",($mod.t - $mod.t0) % 60);
      ...
      await pas.crt.Delay(60000);
      if (Math.floor(($mod.t - $mod.t0) / 60) >= 2) {
        $mod.n = pas.System.Random($mod.r1 + $mod.r2);
        if ($mod.n > $mod.r1) {
          $mod.q2 = $mod.n}
         else $mod.q1 = $mod.n;
      };
    } while (!((Math.abs($mod.q1 - $mod.n) <= $mod.r1) || (Math.abs($mod.q2 - $mod.n) <= $mod.r2)));
    if ((Math.abs($mod.q1 - $mod.n) <= $mod.r1) && (Math.abs($mod.q2 - $mod.n) <= $mod.r2)) {
      if (Math.abs($mod.q1 - $mod.n) > Math.abs($mod.q2 - $mod.n)) {
        $mod.q1 = $mod.n}
       else $mod.q2 = $mod.n;
    } else if (Math.abs($mod.q1 - $mod.n) <= $mod.r1) {
      $mod.q1 = $mod.n}
     else $mod.q2 = $mod.n;
    if ($mod.q1 === $mod.n) {
      pas.crt.ClrScr();
      pas.System.Writeln(Math.floor(($mod.t - $mod.t0) / 60),":",($mod.t - $mod.t0) % 60);
      pas.System.Writeln($mod.t1);
      pas.System.Write($mod.t2);
      pas.crt.GotoXY(40,2);
      $mod.s1 = $mod.s1 + 150;
